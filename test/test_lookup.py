...
class LookupTest:
    ...
    def test_dont_accept_relative_outside_of_root_via_backslash_include(self):
        """test that an include / namespace URI using backslash path
        separators can't bypass the path traversal check"""
        with tempfile.TemporaryDirectory() as base:
            tmpl_dir = os.path.join(base, "app", "templates")
            os.makedirs(tmpl_dir)
            with open(os.path.join(tmpl_dir, "index.html"), "w") as f:
